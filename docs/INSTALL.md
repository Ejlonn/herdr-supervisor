...
Run `./install.sh --dry-run` first, then `./install.sh` as the target non-root user. Both refuse to proceed, before creating any file, unless the Herdr executable exists at `~/.local/bin/herdr`. The source tree is copied to `~/.local/lib/herdr-supervisor`, launchers to `~/.local/bin`, examples and documentation to `~/.local/share/doc/herdr-supervisor`, and inactive units to `~/.config/systemd/user`.
